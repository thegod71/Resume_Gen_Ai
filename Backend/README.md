...
=
------AI----
now to use of ai we use gemini API key on google studio
==> To handle file in the backend (the resume which is come) we use multer (npm i multer)
and to read the contain of pdf we another package need pdf parse(npm i pdf-parse)

=> to set-up multer in middleware folder
